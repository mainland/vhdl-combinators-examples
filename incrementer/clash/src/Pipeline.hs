...
-- | Parallel iteration (pipeline bubbles are not eliminated).
piter' :: forall n a dom n0 . (HiddenClockResetEnable dom, KnownNat n, NFDataX a, (n0 + 1) ~ n)
       => (a -> Index n -> a)
       -> Pipeline dom a a
piter' f = mealy step (undefined, replicate (SNat :: SNat n) False)
  where
    step :: (Vec n a, Vec n Valid)
         -> ValidReady a
         -> ((Vec n a, Vec n Valid), ValidReady a)
    step (x_,v_) (in_data,in_valid,out_ready) = ((x',v'), (out_data,out_valid,in_ready))
      where
        v' :: Vec n Valid
        v' = if out_ready then in_valid +>> v_ else v_

        x' :: Vec n a
        x' = if out_ready then imap (flip f) (in_data +>> x_) else x_
